...
function MessagesExperience() {
  ...
  return (
    <div className="min-h-screen bg-[#FBFCFF] pb-28 text-[#09193B] lg:pb-10">
      <motion.main initial={reduceMotion ? false : { opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={entrance} className="mx-auto max-w-[920px] px-4 pb-8 pt-[max(1rem,env(safe-area-inset-top))] sm:px-6 lg:px-8">
        <section className="flex min-h-[72px] items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-2 text-[11px] font-medium text-[#71809F] sm:text-[12px]">{tx("Your conversations, all in one place", "သင့်စကားပြောမှုအားလုံး တစ်နေရာတည်းတွင်")}</p>
          </div>
          <div className="flex shrink-0 items-center gap-2.5">
            <button type="button" onClick={() => setSearchOpen((current) => !current)} aria-expanded={searchOpen} aria-label={tx("Search conversations", "စကားပြောမှုများရှာရန်")} className="grid size-12 place-items-center rounded-full border border-white bg-white text-[#09193B] shadow-[0_8px_24px_rgba(29,57,99,.08)] transition-colors hover:text-[#0866F5]"><Search className="size-5" /></button>
          </div>
        </section>

        <AnimatePresence>{searchOpen && <motion.label initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 44 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, height: 0 }} className="mt-3 flex items-center gap-2 overflow-hidden rounded-full border border-[#DCE4F0] bg-white px-4 shadow-sm transition-[border-color,box-shadow] focus-within:border-[#9DBCE8] focus-within:shadow-[0_0_0_3px_rgba(0,87,217,.08)]"><Search className="size-4 text-[#0866F5]" /><span className="sr-only">{tx("Search conversations", "စကားပြောမှုများရှာရန်")}</span><input data-focus-ring="parent" autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder={tx("Search conversations…", "စကားပြောမှုများရှာပါ…")} className="min-w-0 flex-1 bg-transparent text-[11px] outline-none placeholder:text-[#8592A8]" /></motion.label>}</AnimatePresence>

        <div className="hide-scrollbar -mx-4 mt-5 overflow-x-auto px-4 pb-1 sm:-mx-6 sm:px-6">
          <div role="tablist" aria-label={tx("Message categories", "စာအမျိုးအစားများ")} className="grid min-w-[472px] grid-cols-5 rounded-[25px] border border-[#E0E6F0] bg-white p-1.5 shadow-[0_10px_28px_rgba(29,57,99,.07)]">
            {filterItems.map(({ id, label, icon: Icon, count }) => {
              const selected = filter === id;
              return <button key={id} type="button" role="tab" aria-selected={selected} onClick={() => setFilter(id)} className={cn("relative flex h-12 min-w-0 items-center justify-center gap-1.5 rounded-[19px] px-1.5 text-[10px] font-semibold transition-colors", selected ? "bg-[#EEF4FF] text-[#0866F5]" : "text-[#17274A] hover:bg-[#F7F9FC]")}><Icon className="size-[17px] shrink-0" /><span className="truncate">{label}</span>{count ? <span className={cn("grid size-5 shrink-0 place-items-center rounded-full text-[8px] font-bold", selected ? "bg-[#0866F5] text-white" : "bg-[#0866F5] text-white")}>{count}</span> : null}</button>;
            })}
          </div>
        </div>

        <section className="mt-4 rounded-[22px] border border-[#E2E8F2] bg-white p-3.5 shadow-[0_10px_28px_rgba(29,57,99,.065)]" aria-labelledby="featured-message-homes">
          <div className="flex h-8 items-center justify-between gap-4 px-1">
            <h2 id="featured-message-homes" className="inline-flex items-center gap-2 text-[12px] font-bold text-[#0A2B7A]"><span className="grid size-5 place-items-center rounded-full bg-[#0866F5] text-white"><Star className="size-3 fill-current" /></span>{tx("Featured", "အထူးရွေးချယ်ထား")}</h2>
            <Link href="/search?purpose=rent" className="inline-flex h-10 items-center gap-1 text-[10px] font-semibold text-[#0866F5]">{tx("View all", "အားလုံးကြည့်")}<ChevronRight className="size-4" /></Link>
          </div>
        ...
        <section className="mt-4 overflow-hidden rounded-[22px] border border-[#E3E9F2] bg-white shadow-[0_10px_28px_rgba(29,57,99,.06)]" aria-label={tx("Conversation list", "စကားပြောမှုစာရင်း")}>
          ...
          {visibleMessages.length === 0 && filter !== "system" && <div className="px-6 py-12 text-center"><MessageCircle className="mx-auto size-7 text-[#0866F5]" /><p className="mt-3 text-[11px] font-semibold">{tx("No conversations in this category", "ဤအမျိုးအစားတွင် စကားပြောမှုမရှိသေးပါ")}</p></div>}
        </section>
      </motion.main>
...
function FeaturedHome({ property, index, reduceMotion }: { property: Property; index: number; reduceMotion: boolean }) {
  ...
  return (
    <motion.article initial={reduceMotion ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: reduceMotion ? 0 : index * 0.06, duration: reduceMotion ? 0 : 0.32 }} className="w-[150px] shrink-0 snap-start overflow-hidden rounded-[16px] border border-[#E4EAF2] bg-white shadow-[0_6px_18px_rgba(29,57,99,.07)]">
      <div className="relative h-[94px] overflow-hidden bg-[#EEF3F8]">
        ...
        <button type="button" aria-label={tx("Save property", "အိမ်သိမ်းရန်")} className="absolute right-2 top-2 z-20 grid size-8 place-items-center rounded-full border border-white/80 bg-white/90 text-[#10224A] shadow-sm backdrop-blur"><Heart className="size-4" /></button>
      ...
      <Link href={`/properties/${property.id}`} className="block p-2.5">
        <h3 className="line-clamp-2 min-h-8 text-[11px] font-semibold leading-4 tracking-[-0.02em] text-[#0B1D41]">{property.title}</h3>
        <p className="mt-1.5 truncate text-[10px] font-semibold text-[#0866F5]">{formatPropertyPrice(property, isMyanmar ? "my" : "en")}<span className="text-[7px] font-medium">{property.purpose === "rent" ? tx(" / month", " / လ") : ""}</span></p>
        <p className="mt-2 flex items-center gap-1.5 text-[8px] font-medium text-[#71809A]"><span className="size-2 rounded-full bg-[#18C767]" />{tx("You inquired", "သင်မေးမြန်းထားသည်")}</p>
      </Link>
    </motion.article>
  );
}
...
function ConversationCard({ conversation, active, index, reduceMotion, onClick }: { conversation: UserConversation; active: boolean; index: number; reduceMotion: boolean; onClick: () => void }) {
  ...
  return (
    <motion.button
      ...
      whileHover={reduceMotion ? undefined : { backgroundColor: "#F8FAFD" }}
      whileTap={reduceMotion ? undefined : { scale: 0.995 }}
      className={cn("group relative grid w-full grid-cols-[58px_minmax(0,1fr)_62px] items-center gap-3 border-b border-[#E8EDF4] px-4 py-3.5 text-left last:border-b-0", active && "bg-[#F8FAFF]")}
      aria-label={tx(`Open conversation about ${property.title}`, `${property.title} အကြောင်း စကားပြောခန်းဖွင့်ရန်`)}
    >
      {conversation.unread && <span className="absolute left-3 top-1/2 size-2 -translate-y-1/2 rounded-full bg-[#0866F5]" aria-label={tx("Unread message", "မဖတ်ရသေးသောစာ")} />}
      <span className={cn("relative grid size-[58px] place-items-center overflow-hidden rounded-full border-2 border-white text-[13px] font-bold shadow-[0_5px_16px_rgba(29,57,99,.12)]", index === 0 ? "bg-[#E7EEF7]" : index === 1 ? "bg-[#EAF6F2] text-[#146B53]" : "bg-[#F8ECE7] text-[#8A4C36]")}>
      ...
      <span className="min-w-0">
        <strong className="block truncate text-[12px] font-bold tracking-[-0.02em] text-[#0B1D41]">{conversation.contact} <span className="font-semibold">({role})</span></strong>
      ...
      <span className="flex h-full min-w-0 flex-col items-end justify-center gap-2">
        ...
        <span className="flex items-center gap-2">{conversation.unread && <span className="grid size-6 place-items-center rounded-full bg-[#0866F5] text-[9px] font-bold text-white">{index === 0 ? 2 : 1}</span>}<ChevronRight className="size-4 text-[#A7B2C4]" /></span>
      </span>
    </motion.button>
  );
}
...
function SystemConversation({ icon, title, preview, time, count, verified = false }: { icon: "bell" | "gift"; title: string; preview: string; time: string; count?: number; verified?: boolean }) {
  ...
  return (
    <div className="grid grid-cols-[58px_minmax(0,1fr)_62px] items-center gap-3 border-b border-[#E8EDF4] px-4 py-3.5 last:border-b-0">
      <span className={cn("grid size-[58px] place-items-center rounded-full", icon === "bell" ? "bg-[#EEF4FF] text-[#0866F5]" : "bg-[#F5F0FF] text-[#7553E7]")}><Icon className="size-6" /></span>
      <span className="min-w-0"><strong className="flex items-center gap-1.5 truncate text-[12px] font-bold text-[#0B1D41]">{title}{verified && <ShieldCheck className="size-4 fill-[#0866F5] text-white" />}</strong><span className="mt-1 line-clamp-2 text-[10px] leading-[15px] text-[#667694]">{preview}</span></span>
