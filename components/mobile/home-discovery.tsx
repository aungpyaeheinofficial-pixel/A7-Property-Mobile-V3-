          ...
          <div className="relative min-h-[540px] overflow-hidden rounded-[var(--radius-sheet)] bg-a7-navy shadow-[var(--shadow-soft)] sm:min-h-[590px] lg:min-h-[590px]">
            ...
            <motion.div initial={reduceMotion ? false : "hidden"} animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: reduceMotion ? 0 : 0.09, delayChildren: reduceMotion ? 0 : 0.16 } } }} className="relative z-10 flex min-h-[540px] max-w-[720px] flex-col px-5 pb-5 pt-6 text-white sm:min-h-[590px] sm:px-10 sm:pb-10 sm:pt-10 lg:min-h-[590px] lg:px-14 lg:py-12">
              ...
              <div className="mt-auto">
                ...
                <motion.form variants={{ hidden: { opacity: 0, y: 16 }, show: { opacity: 1, y: 0, transition: reduceMotion ? { duration: 0 } : a7Motion.slow } }} onSubmit={submitSearch} className="mt-5 space-y-2.5 sm:mt-6 sm:max-w-[640px]">
                  <div className="flex min-h-16 items-center gap-2 rounded-full border border-white/70 bg-white p-2 pl-5 shadow-[0_14px_36px_rgba(15,27,45,.22)] transition-[border-color,box-shadow] duration-200 focus-within:border-[#9FC4FF] focus-within:shadow-[0_16px_42px_rgba(15,27,45,.26),0_0_0_4px_rgba(0,87,217,.12)]">
                    <Search className="size-5 shrink-0 text-[#0057D9]" />
                    <input value={query} onChange={(event) => setQuery(event.target.value)} aria-label={tx("Find your next home", "သင့်နောက်အိမ်ကို ရှာပါ")} placeholder={tx("Township, landmark or property", "မြို့နယ်၊ နေရာ သို့မဟုတ် အိမ်အမျိုးအစား")} className="h-12 min-w-0 flex-1 bg-transparent px-1 text-[14px] text-[#111827] outline-none placeholder:text-[#929A96]" />
                    <button type="submit" className="grid size-12 shrink-0 place-items-center rounded-full bg-[#0057D9] text-white shadow-[0_7px_18px_rgba(0,87,217,.28)] transition-[background-color,transform,box-shadow] duration-200 hover:bg-[#0049B8] hover:shadow-[0_9px_22px_rgba(0,87,217,.32)] active:scale-[.96]" aria-label={tx("Search homes", "အိမ်များရှာရန်")}><Search className="size-[19px]" /></button>
                  </div>
                  <div className="grid grid-cols-[1fr_1fr_auto] gap-1.5 rounded-full border border-white/55 bg-white/94 p-1.5 shadow-[0_10px_26px_rgba(15,27,45,.15)] backdrop-blur-xl">
                    <button type="button" onClick={() => setPurpose("rent")} aria-pressed={purpose === "rent"} className={`h-11 rounded-full px-4 text-[11px] font-semibold transition-[background-color,color,box-shadow] duration-200 ${purpose === "rent" ? "bg-[#EEF5FC] text-[#0057D9] shadow-[inset_0_0_0_1px_rgba(0,87,217,.08)]" : "text-[#5F6965] hover:bg-[#F6F7F8]"}`}>{tx("Rent", "ငှားရန်")}</button>
                    <button type="button" onClick={() => setPurpose("sale")} aria-pressed={purpose === "sale"} className={`h-11 rounded-full px-4 text-[11px] font-semibold transition-[background-color,color,box-shadow] duration-200 ${purpose === "sale" ? "bg-[#EEF5FC] text-[#0057D9] shadow-[inset_0_0_0_1px_rgba(0,87,217,.08)]" : "text-[#5F6965] hover:bg-[#F6F7F8]"}`}>{tx("Buy", "ဝယ်ရန်")}</button>
                    <Link href={`/search?purpose=${purpose}`} className="flex h-11 min-w-11 items-center justify-center gap-1.5 rounded-full px-3 text-[10px] font-semibold text-[#5F6965] transition-colors duration-200 hover:bg-[#F6F7F8] hover:text-[#0057D9]" aria-label={tx("Open search filters", "ရှာဖွေမှုစစ်ထုတ်ရန်")}><SlidersHorizontal className="size-4" /><span className="hidden sm:inline">{tx("Filters", "စစ်ထုတ်")}</span></Link>
                  </div>
                </motion.form>
              </div>
            </motion.div>
