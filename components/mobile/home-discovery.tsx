...
function HomeDiscovery() {
  ...
  const revealTransition = reduceMotion ? { duration: 0 } : { type: "spring" as const, stiffness: 92, damping: 18 };

  return (
    <div className="min-h-screen overflow-hidden bg-[#F8F3F0] pb-28 text-[#111827] lg:pb-0">
      <header className="sticky top-0 z-50 border-b border-[#E6E3DC]/80 bg-[#F8F3F0]/94 backdrop-blur-2xl">
        <div className="mx-auto flex h-[70px] max-w-[1240px] items-center gap-3 px-4 sm:px-6 lg:h-[78px] lg:px-8">
          <motion.div initial={reduceMotion ? false : { opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reduceMotion ? 0 : 0.65, ease: [0.22, 1, 0.36, 1] }}><Link href="/" aria-label="A7 Property home"><A7Brand /></Link></motion.div>
          <motion.div initial={reduceMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: reduceMotion ? 0 : 0.2, duration: 0.5 }} className="ml-auto flex items-center gap-1">
            ...
            <Link href={user ? "/profile" : "/sign-in"} className="grid size-11 place-items-center rounded-full border border-[#DCD9D1] bg-white text-[#014BAA] shadow-sm" aria-label={user ? tx("Open profile", "ပရိုဖိုင်ဖွင့်ရန်") : tx("Sign in", "အကောင့်ဝင်ရန်")}><UserRound className="size-[19px]" /></Link>
          </motion.div>
        </div>
      </header>
      ...
      <AnimatePresence>
        {stickySearch && (
          <motion.form initial={reduceMotion ? false : { opacity: 0, y: -12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} transition={{ duration: reduceMotion ? 0 : 0.24, ease: [0.22, 1, 0.36, 1] }} onSubmit={submitSearch} className="fixed inset-x-4 top-[78px] z-40 flex h-14 items-center gap-2 rounded-[16px] border border-[#E2DFD7] bg-white/97 p-1.5 pl-4 shadow-[0_8px_28px_rgba(15,23,42,.12)] backdrop-blur-xl lg:hidden" role="search">
            <Search className="size-[18px] shrink-0 text-[#014BAA]" />
            ...
            <button type="submit" className="grid size-11 shrink-0 place-items-center rounded-[13px] bg-[#014BAA] text-white" aria-label={tx("Search homes", "အိမ်များရှာရန်")}><ArrowRight className="size-4" /></button>
          </motion.form>
        )}
      </AnimatePresence>

      <motion.main initial={reduceMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: reduceMotion ? 0 : 0.55 }}>
        <section ref={heroRef} className="relative mx-auto max-w-[1320px] px-3 pt-3 sm:px-6 sm:pt-6 lg:px-8">
          <div className="relative min-h-[540px] overflow-hidden rounded-[24px] bg-[#003F91] sm:min-h-[590px] lg:min-h-[590px]">
            <motion.div style={{ y: reduceMotion ? 0 : heroImageY, willChange: "transform" }} className="absolute -inset-y-12 inset-x-0">
              <motion.div initial={reduceMotion ? false : { scale: 1.05 }} animate={{ scale: 1 }} transition={{ duration: reduceMotion ? 0 : 0.8, ease: [0.22, 1, 0.36, 1] }} className="absolute inset-0">
            ...
            <motion.div initial={reduceMotion ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: reduceMotion ? 0 : 0.8 }} className="absolute inset-0 bg-[linear-gradient(180deg,rgba(16,34,54,.24),rgba(16,34,54,.78)),linear-gradient(90deg,rgba(16,34,54,.86),rgba(16,34,54,.2))]" />
            ...
            <motion.div initial={reduceMotion ? false : "hidden"} animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: reduceMotion ? 0 : 0.09, delayChildren: reduceMotion ? 0 : 0.16 } } }} className="relative z-10 flex min-h-[540px] max-w-[720px] flex-col px-5 pb-5 pt-6 text-white sm:min-h-[590px] sm:px-10 sm:pb-10 sm:pt-10 lg:min-h-[590px] lg:px-14 lg:py-12">
              ...
              <div className="mt-auto">
                ...
                <motion.form variants={{ hidden: { opacity: 0, y: 24, scale: 0.98 }, show: { opacity: 1, y: 0, scale: 1, transition: reduceMotion ? { duration: 0 } : { type: "spring", stiffness: 100, damping: 17 } } }} onSubmit={submitSearch} className="mt-5 rounded-[14px] border border-white/55 bg-white p-2 shadow-[0_8px_28px_rgba(15,23,42,.14)] sm:mt-6 sm:max-w-[640px]">
                  <div className="flex items-center gap-2">
                    <MapPin className="ml-2 size-5 shrink-0 text-[#014BAA]" />
                    ...
                    <button type="submit" className="grid size-12 shrink-0 place-items-center rounded-[14px] bg-[#014BAA] text-white shadow-[0_4px_20px_rgba(0,0,0,.08)] transition-colors duration-200 hover:bg-[#003F91]" aria-label={tx("Search homes", "အိမ်များရှာရန်")}><Search className="size-5" /></button>
                  </div>
                  <div className="grid grid-cols-3 gap-1.5 border-t border-[#ECE9E3] px-1 pt-2">
                    <button type="button" onClick={() => setPurpose("rent")} className={`h-11 rounded-[10px] text-[10px] font-semibold ${purpose === "rent" ? "bg-[#EEF5FC] text-[#014BAA]" : "text-[#6C7571]"}`}>{tx("Rent", "ငှားရန်")}</button>
                    <button type="button" onClick={() => setPurpose("sale")} className={`h-11 rounded-[10px] text-[10px] font-semibold ${purpose === "sale" ? "bg-[#EEF5FC] text-[#014BAA]" : "text-[#6C7571]"}`}>{tx("Buy", "ဝယ်ရန်")}</button>
        ...
        <section className="mx-auto max-w-[1240px] px-4 py-10 sm:px-6 sm:py-12 lg:px-8 lg:py-16">
          ...
          <div className="-mx-4 mt-6 flex snap-x gap-3 overflow-x-auto px-4 pb-3 sm:mx-0 sm:grid sm:grid-cols-4 sm:px-0">
            {townshipMeta.map((item, index) => {
              ...
              return (
                <motion.div key={item.name} initial={reduceMotion ? false : { opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.28 }} transition={{ delay: reduceMotion ? 0 : index * 0.08, duration: reduceMotion ? 0 : 0.48, ease: [0.22, 1, 0.36, 1] }} className="min-w-[168px] snap-start sm:min-w-0">
        ...
        <section className="bg-[#F1F0EB] py-12 lg:py-16">
          <div className="mx-auto max-w-[1240px] px-4 sm:px-6 lg:px-8">
            ...
            <div className="-mx-4 mt-7 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-5 sm:mx-0 sm:grid sm:grid-cols-2 sm:px-0 lg:grid-cols-3">
              {recommended.slice(0, 3).map((property, index) => (
                <motion.div key={property.id} initial={reduceMotion ? false : { opacity: 0.88, scale: 0.96 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ amount: 0.65, margin: "0px -16% 0px -16%" }} transition={{ delay: reduceMotion ? 0 : index * 0.04, duration: reduceMotion ? 0 : 0.38, ease: [0.22, 1, 0.36, 1] }} className="min-w-[86vw] snap-center sm:min-w-0">
        ...
        <motion.section initial={reduceMotion ? false : { opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: reduceMotion ? 0 : 0.48, ease: [0.22, 1, 0.36, 1] }} className="mx-auto max-w-[1240px] px-4 pb-14 sm:px-6 lg:px-8 lg:pb-20">
          <div className="relative overflow-hidden rounded-[20px] border border-[#E5E7EB] bg-white px-6 py-7 text-[#111827] shadow-[0_4px_20px_rgba(0,0,0,.08)] sm:px-8 sm:py-8 lg:grid lg:grid-cols-[1fr_auto] lg:items-center lg:gap-10">
            <div className="relative"><span className="inline-flex items-center gap-2 rounded-full bg-[#EEF5FC] px-3 py-1.5 text-[10px] font-semibold text-[#014BAA]"><Sparkles className="size-3.5" />A7 Assistant</span><h2 className="mt-4 text-[30px] font-semibold leading-[1.15] tracking-[-0.045em] sm:text-[38px]">{tx("Tell us what you need.", "သင်လိုချင်တာကို ပြောပါ။")}</h2><p className="mt-3 max-w-[650px] text-[13px] leading-6 text-[#64748B]">{tx("“I need a 2 bedroom condo near Hledan under 800,000 MMK.”", "“လှည်းတန်းအနီး ၈ သိန်းအောက် အိပ်ခန်း ၂ ခန်းကွန်ဒို လိုချင်တယ်။”")}</p></div>
            <Link href="/assistant" className="relative mt-6 inline-flex h-12 items-center gap-2 rounded-[14px] bg-[#014BAA] px-5 text-[12px] font-semibold text-white transition-colors duration-200 hover:bg-[#003F91] lg:mt-0"><Sparkles className="size-5" />{tx("Ask A7", "A7 ကိုမေးမယ်")}<ArrowRight className="size-4" /></Link>
          </div>
        </motion.section>
      </motion.main>
      ...
      <AnimatePresence mode="wait">
        {!assistantOpen ? (
          <motion.button key="ask-a7-button" layoutId="ask-a7-home" type="button" onClick={() => setAssistantOpen(true)} animate={reduceMotion ? undefined : { scale: [1, 1.018, 1] }} transition={reduceMotion ? { duration: 0 } : { duration: 4.8, repeat: Infinity, ease: "easeInOut" }} className="fixed bottom-[88px] right-4 z-[70] inline-flex h-11 items-center gap-2 rounded-full border border-white bg-[#014BAA] px-4 text-[11px] font-semibold text-white shadow-[0_8px_24px_rgba(1,75,170,.24)] lg:bottom-6" aria-label={tx("Ask A7 assistant about a home", "အိမ်အကြောင်း A7 အကူကိုမေးရန်")}><Sparkles className="size-4" />{tx("Ask A7", "A7 ကိုမေး")}</motion.button>
        ) : (
