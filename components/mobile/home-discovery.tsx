...
function HomeDiscovery() {
  ...
  return (
    <div className="min-h-screen overflow-x-clip bg-[#EAF4FF] pb-[116px] text-[#101828]">
      <motion.main
      ...
      >
        <div className="mx-auto w-full max-w-[920px]">
          <motion.header variants={reveal(reduceMotion)} className="flex h-[64px] items-center gap-3" aria-label={tx("A7 Property home header", "A7 Property ပင်မခေါင်းစီး")}>
            <Link href="/" className="inline-flex min-w-0 items-center gap-2.5" aria-label="A7 Property home">
              <span className="relative size-12 shrink-0 overflow-hidden rounded-[15px] shadow-[0_8px_24px_rgba(16,24,40,.1)]"><Image src="/images/brand/a7-property-logo.jpg" alt="" fill sizes="48px" className="scale-[2.55] object-contain" /></span>
              <span className="truncate text-[22px] font-semibold tracking-[-0.045em] text-[#123B73]">A7 <span className="text-[#101828]">Property</span></span>
            </Link>
          ...
          <motion.form variants={reveal(reduceMotion)} onSubmit={submitSearch} role="search" className="mt-4 flex h-[64px] items-center gap-2 rounded-full border border-[#D0DEF0] bg-[#F8FBFF] p-1.5 pl-5 shadow-[0_10px_30px_rgba(24,48,86,.06)] focus-within:border-[#4DA3FF] focus-within:ring-4 focus-within:ring-[#123B73]/8">
            <Search className="size-[22px] shrink-0 text-[#667085]" />
            <input data-focus-ring="parent" value={query} onChange={(event) => setQuery(event.target.value)} aria-label={tx("Search for location or property", "နေရာ သို့မဟုတ် အိမ်ရှာရန်")} placeholder={tx("Search for location, property or keyword…", "နေရာ၊ အိမ် သို့မဟုတ် စာလုံးရှာပါ…")} className="min-w-0 flex-1 bg-transparent px-2 text-[13px] font-medium text-[#101828] outline-none placeholder:text-[#667085] sm:text-[14px]" />
            <Link href={searchHref} className="grid size-[52px] shrink-0 place-items-center rounded-[18px] bg-[#123B73] text-white shadow-[0_8px_22px_rgba(77,163,255,.24)] transition-transform active:scale-95" aria-label={tx("Open all filters", "စစ်ထုတ်မှုအားလုံးဖွင့်ရန်")}><SlidersHorizontal className="size-[21px]" /></Link>
          </motion.form>
          ...
          <motion.section variants={reveal(reduceMotion)} className="relative mt-2 h-[clamp(258px,52vw,500px)] overflow-hidden rounded-[28px] bg-[#EAF4FF]" aria-labelledby="home-hero-title">
            <ProgressiveImage src="/images/properties/a7-yangon-daylight-hero.png" alt="Modern curved glass residence overlooking Yangon" fill priority sizes="(max-width: 920px) 100vw, 920px" className="object-cover object-center" skeletonClassName="bg-[#DCEBFF]" />
            <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(248,251,255,.96)_0%,rgba(248,251,255,.86)_27%,rgba(248,251,255,.32)_48%,rgba(248,251,255,0)_68%)]" />
            <div className="absolute inset-y-0 left-0 z-10 flex w-1/2 max-w-[440px] flex-col justify-center px-6 pb-6 sm:px-10">
              <span className="w-fit rounded-[12px] border border-[#123B73]/14 bg-[#F8FBFF]/65 px-3 py-1.5 text-[9px] font-semibold text-[#123B73] shadow-sm backdrop-blur-xl">{tx("Find Your Perfect Space", "သင့်အတွက် ပြီးပြည့်စုံသောနေရာ")}</span>
              <h1 id="home-hero-title" className="mt-4 text-[clamp(34px,7vw,66px)] font-semibold leading-[1.03] tracking-[-0.055em] text-[#101828]">{tx("Discover Your Next Home", "သင့်အိမ်အသစ်ကို ရှာဖွေပါ")} <Sparkles className="inline size-[.76em] -translate-y-0.5 fill-[#123B73] text-[#123B73]" /></h1>
              <p className="mt-4 max-w-[330px] text-[clamp(10px,2.2vw,15px)] font-medium leading-[1.45] text-[#101828]">{tx("Explore verified properties in Myanmar with A7 Property.", "A7 Property ဖြင့် မြန်မာနိုင်ငံရှိ စိစစ်ပြီးသောအိမ်များကို ရှာဖွေပါ။")}</p>
            </div>
          </motion.section>

          <motion.nav variants={reveal(reduceMotion)} className="relative z-20 mx-3 -mt-9 grid grid-cols-5 gap-1 rounded-[24px] border border-[#D0DEF0] bg-[#F8FBFF]/94 p-3 shadow-[0_18px_42px_rgba(26,55,96,.12)] backdrop-blur-2xl sm:mx-6 sm:-mt-12 sm:p-4" aria-label={tx("Browse by property type", "အိမ်အမျိုးအစားဖြင့်ရှာရန်")}>
        ...
        <motion.section variants={reveal(reduceMotion)} className="mx-auto mt-4 w-full max-w-[920px]" aria-labelledby="recommendation-title">
          ...
          <div className="mt-4 grid min-h-[132px] grid-cols-[1fr_auto] items-center gap-3 overflow-hidden rounded-[24px] border border-[#DDE8F8] bg-[linear-gradient(105deg,#F8FBFF_0%,#DCEBFF_100%)] p-4 shadow-[0_12px_30px_rgba(36,76,130,.07)] sm:p-5">
            <div className="flex min-w-0 items-center gap-3">
              <span className="relative grid size-14 shrink-0 place-items-center rounded-full border border-white bg-[#F8FBFF]/76 text-[24px] font-semibold text-[#123B73] shadow-[0_8px_22px_rgba(40,83,140,.08)]">AI<Sparkles className="absolute right-1 top-1 size-3 fill-[#123B73]" /></span>
              <div className="min-w-0">
                <p className="text-[9px] font-medium text-[#667085]">{tx("Smart Recommendation", "စမတ်အကြံပြုချက်")}</p>
                <h2 className="mt-1 max-w-[260px] text-[14px] font-semibold leading-5 tracking-[-0.02em] text-[#101828] sm:text-[17px] sm:leading-6">{tx("3 new properties match your preferences", "သင့်စိတ်ကြိုက်နှင့် ကိုက်ညီသည့်အိမ်သစ် ၃ လုံးရှိသည်")}</h2>
                <Link href="/assistant" className="mt-3 inline-flex h-9 items-center rounded-[12px] bg-[#123B73] px-4 text-[9px] font-semibold text-white shadow-[0_7px_16px_rgba(77,163,255,.2)]">{tx("View Now", "ယခုကြည့်မယ်")}</Link>
              </div>
            </div>
            <div className="hidden items-center gap-2 min-[430px]:flex">
              <div className="flex -space-x-5">
                {promoted.slice(0, 2).map((property, index) => <span key={property.id} className="relative h-[66px] w-[82px] overflow-hidden rounded-[14px] border-2 border-white bg-[#DCEBFF] shadow-[0_8px_18px_rgba(29,63,109,.12)]" style={{ zIndex: 2 - index, transform: `translateY(${index * -5}px)` }}><Image src={property.images[0]} alt="" fill sizes="82px" className="object-cover" /></span>)}
              </div>
              <span className="grid size-12 place-items-center rounded-full bg-[#F8FBFF] text-center text-[9px] font-semibold leading-3 text-[#101828] shadow-sm">+3<br />{tx("More", "ထပ်မံ")}</span>
            </div>
          </div>
        </motion.section>
      </motion.main>
...
function HeaderAction({ href, label, count, children }: { href: string; label: string; count?: number; children: ReactNode }) {
  return (
    <Link href={href} aria-label={label} className="relative grid size-12 place-items-center rounded-full border border-[#D0DEF0] bg-[#F8FBFF] text-[#101828] shadow-[0_8px_24px_rgba(28,55,92,.07)] transition-transform active:scale-95">
      {children}
      {count ? <span className="absolute -right-0.5 -top-0.5 grid size-[18px] place-items-center rounded-full bg-[#123B73] text-[8px] font-bold text-white ring-2 ring-white">{count}</span> : null}
    </Link>
  );
}
...
function ModeButton({ selected, onClick, icon: Icon, label }: { selected: boolean; onClick: () => void; icon: ComponentType<{ className?: string }>; label: string }) {
  return (
    <button type="button" role="tab" aria-selected={selected} onClick={onClick} className={cn("inline-flex h-12 min-w-0 items-center justify-center gap-2 rounded-[18px] border px-2 text-[11px] font-semibold transition-[background-color,border-color,box-shadow,transform] active:scale-[.98] sm:px-4 sm:text-[12px]", selected ? "border-[#123B73] bg-[#123B73] text-white shadow-[0_8px_22px_rgba(77,163,255,.2)]" : "border-[#D0DEF0] bg-[#F8FBFF] text-[#667085] shadow-[0_6px_18px_rgba(28,55,92,.03)] hover:border-[#4DA3FF]")}>
...
function QuickCategoryItem({ category, mode, isMyanmar }: { category: QuickCategory; mode: HomeMode; isMyanmar: boolean }) {
  ...
  return (
    <Link href={buildSearchHref(mode, category.type)} className="group flex min-w-0 flex-col items-center gap-2 rounded-[16px] py-1 text-[#101828]">
      <span className="grid size-11 place-items-center rounded-full bg-[#DCEBFF] text-[#123B73] transition-[transform,background-color] group-hover:-translate-y-0.5 group-hover:bg-[#DCEBFF]"><Icon className="size-[21px]" strokeWidth={2.2} /></span>
...
function SectionHeading({ id, title, action, href }: { id: string; title: string; action: string; href: string }) {
  return (
    <div className="flex items-center justify-between gap-4">
      <h2 id={id} className="text-[18px] font-semibold tracking-[-0.03em] text-[#101828]">{title}</h2>
      <Link href={href} className="inline-flex h-11 items-center gap-1.5 text-[11px] font-semibold text-[#123B73]">{action}<ArrowRight className="size-4" /></Link>
    </div>
  );
}
...
function PromotedCard({ property, saved, onToggleSaved, tx, priority = false }: { property: Property; saved: boolean; onToggleSaved: (property: Property) => void; tx: (english: string, myanmar: string) => string; priority?: boolean }) {
  return (
    <article className="overflow-hidden rounded-[22px] border border-[#D0DEF0] bg-[#F8FBFF] shadow-[0_12px_30px_rgba(27,52,88,.09)]">
      <div className="relative h-[190px] overflow-hidden bg-[#DCEBFF]">
        ...
        <span className="absolute left-3 top-3 z-20 inline-flex h-8 items-center gap-1.5 rounded-full border border-white/85 bg-[#F8FBFF]/90 px-3 text-[9px] font-semibold text-[#101828] shadow-sm backdrop-blur-md"><Check className="size-3.5 text-[#123B73]" />{formatPropertyType(property.property_type)} · {tx("Verified", "စိစစ်ပြီး")}</span>
        <button type="button" onClick={() => onToggleSaved(property)} className="absolute right-3 top-3 z-20 grid size-11 place-items-center rounded-full border border-white/90 bg-[#F8FBFF]/92 text-[#101828] shadow-sm backdrop-blur-md transition-transform active:scale-95" aria-label={saved ? tx(`Remove ${property.title} from saved homes`, `${property.title} ကိုသိမ်းထားမှုမှဖယ်ရန်`) : tx(`Save ${property.title}`, `${property.title} ကိုသိမ်းရန်`)} aria-pressed={saved}><Heart className={cn("size-5", saved && "fill-current text-[#123B73]")} /></button>
      </div>
