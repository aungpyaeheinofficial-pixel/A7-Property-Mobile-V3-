...
function SavedJourney() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] pb-28 text-[#111827] lg:pb-16">
      <header className="sticky top-0 z-50 border-b border-[#E6E3DC]/80 bg-[#FAF8F5]/92 backdrop-blur-2xl">
        <div className="mx-auto flex h-[70px] max-w-[1240px] items-center px-4 sm:px-6 lg:px-8">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <Link href="/search?purpose=rent" className="grid size-11 place-items-center rounded-full border border-[#DDDAD2] bg-white text-[#0057D9] shadow-sm" aria-label={tx("Find homes", "အိမ်ရှာရန်")}><Search className="size-[18px]" /></Link>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-[1240px] px-4 sm:px-6 lg:px-8">
        <section className="pb-8 pt-9 sm:pb-10 sm:pt-12">
          ...
          <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
            ...
            <span className="inline-flex h-9 items-center gap-2 rounded-full bg-[#EEF5FC] px-3 text-[10px] font-semibold text-[#0057D9]"><ShieldCheck className="size-4" />{tx("Private to you", "သင်တစ်ဦးတည်းသာမြင်နိုင်")}</span>
          </div>
        </section>
        ...
        {feature ? (
          <section className="grid overflow-hidden rounded-[20px] border border-[#E6E2DA] bg-white shadow-[0_4px_20px_rgba(0,0,0,.08)] lg:grid-cols-[1.25fr_.75fr]">
            <div className="relative min-h-[330px] overflow-hidden bg-[#E9E7E1] sm:min-h-[440px]">
              ...
              <span className="absolute left-4 top-4 z-20 inline-flex h-8 items-center gap-1.5 rounded-full bg-white/94 px-3 text-[9px] font-semibold text-[#0057D9] shadow-sm backdrop-blur"><ShieldCheck className="size-3.5" />{tx("Verified home", "စိစစ်ထားသောအိမ်")}</span>
              <button type="button" onClick={() => toggleSaved(feature)} className="absolute right-4 top-4 z-20 grid size-11 place-items-center rounded-full bg-white/94 text-[#0057D9] shadow-lg" aria-label={tx("Remove saved home", "သိမ်းထားမှုမှဖယ်ရန်")}><Heart className="size-5 fill-current" /></button>
            ...
            <div className="flex flex-col justify-between p-6 sm:p-8 lg:p-9">
              <div>
                ...
                <p className="mt-4 text-[28px] font-semibold tracking-[-0.045em] text-[#0057D9]">{formatPropertyPrice(feature, isMyanmar ? "my" : "en")}<span className="ml-1 text-[11px] font-normal text-[#78817C]">{feature.purpose === "rent" ? tx("/ month", "/ လ") : ""}</span></p>
        ...
        <div className="grid gap-6 pb-12 lg:grid-cols-2 lg:pb-16">
          <section className="rounded-[20px] border border-[#E5E2DB] bg-white p-5 shadow-[0_4px_20px_rgba(0,0,0,.08)] sm:p-7">
            <div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-full bg-[#EEF5FC] text-[#0057D9]"><BellRing className="size-[19px]" /></span><div><h2 className="text-[22px] font-semibold">{tx("Saved searches", "သိမ်းထားသောရှာဖွေမှုများ")}</h2><p className="mt-1 text-[10px] text-[#737C77]">{tx("New homes, without the constant checking", "အိမ်အသစ်များကို အလိုအလျောက် အသိပေးမည်")}</p></div></div>
            <div className="mt-5 divide-y divide-[#ECE9E3]">{savedSearches.map((search) => <div key={search.id} className="flex items-center gap-3 py-4"><Link href={search.href} className="grid size-10 shrink-0 place-items-center rounded-full bg-[#F3F1EC] text-[#64748B]" aria-label={tx(`Open ${search.title}`, `${search.title} ကိုဖွင့်ရန်`)}><Search className="size-4" /></Link><Link href={search.href} className="min-w-0 flex-1"><strong className="block truncate text-[12px]">{search.title}</strong><small className="mt-1 block truncate text-[9px] text-[#78817C]">{search.detail} · {search.count} {tx("homes", "အိမ်")}</small></Link><button type="button" role="switch" aria-checked={search.notificationsEnabled} onClick={() => toggleAlert(search.id)} className="relative h-11 w-12 shrink-0" aria-label={`${search.title} alerts`}><span className={`absolute inset-x-0 top-2 h-7 rounded-full transition-colors ${search.notificationsEnabled ? "bg-[#0057D9]" : "bg-[#D4D8D4]"}`} /><span className={`absolute top-3 size-5 rounded-full bg-white shadow-sm transition-transform ${search.notificationsEnabled ? "translate-x-6" : "translate-x-1"}`} /></button></div>)}</div>
          </section>

          <section className="rounded-[20px] border border-[#E5E2DB] bg-white p-5 shadow-[0_4px_20px_rgba(0,0,0,.08)] sm:p-7">
            <div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-full bg-[#EEF5FC] text-[#0057D9]"><TrendingDown className="size-[19px]" /></span><div><h2 className="text-[22px] font-semibold">{tx("Price alerts", "ဈေးနှုန်းအသိပေးချက်")}</h2><p className="mt-1 text-[10px] text-[#737C77]">{tx("A good moment to look again", "ပြန်ကြည့်ရန် အချိန်ကောင်း")}</p></div></div>
            <div className="mt-5 space-y-3">{hasPriceDrop.length ? hasPriceDrop.map((property) => <Link key={property.id} href={`/properties/${property.id}`} className="flex items-center gap-3 rounded-[14px] bg-[#FAF8F5] p-3 transition-colors hover:bg-[#EEF5FC]"><div className="relative size-14 shrink-0 overflow-hidden rounded-[14px]"><Image src={property.images[0]} alt="" fill sizes="56px" className="object-cover" /></div><span className="min-w-0 flex-1"><strong className="block truncate text-[12px]">{property.title}</strong><small className="mt-1 block text-[9px] text-[#64748B]">{tx("Price reduced by", "ဈေးနှုန်းလျှော့ထားသည်")} {new Intl.NumberFormat("en-US").format(priceDrops[property.id])} MMK</small></span><Bell className="size-4 text-[#0057D9]" /></Link>) : <p className="rounded-[14px] bg-[#FAF8F5] p-4 text-[11px] text-[#64748B]">{tx("We’ll let you know when a saved home changes price.", "သိမ်းထားသောအိမ် ဈေးပြောင်းသည့်အခါ အသိပေးပါမည်။")}</p>}</div>
          </section>
        </div>
