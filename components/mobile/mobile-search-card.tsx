...
function MobileSearchCard({ property, saved, onToggleSaved, onOpen, priority = false, compared = false, compareDisabled = false, onToggleCompare, variant = "feature" }: MobileSearchCardProps) {
    ...
    return (
      <motion.article initial={reduceMotion ? false : { opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={reduceMotion ? { duration: 0 } : a7Motion.slow} className="group grid min-h-[205px] grid-cols-[minmax(0,1.05fr)_minmax(138px,.95fr)] overflow-hidden rounded-[24px] border border-[#E2E7EF] bg-white shadow-[0_10px_28px_rgba(26,52,88,.07)]">
        ...
        <div className="relative min-h-[205px] overflow-hidden bg-[#EEF3F8]">
          ...
          <motion.button type="button" onClick={toggleSaved} aria-label={saved ? tx(`Remove ${property.title} from saved homes`, `${property.title} ကို သိမ်းထားမှုမှဖယ်ရန်`) : tx(`Save ${property.title}`, `${property.title} ကို သိမ်းရန်`)} aria-pressed={saved} animate={reduceMotion ? undefined : saved ? { scale: [1, 1.2, 1] } : { scale: 1 }} transition={reduceMotion ? { duration: 0 } : a7Motion.base} className="absolute right-3 top-3 z-20 grid size-11 place-items-center rounded-full border border-white/85 bg-white/92 text-[#183153] shadow-sm backdrop-blur-md"><Heart className={cn("size-[19px]", saved && "fill-current text-[#0A67FF]")} /></motion.button>
          {onToggleCompare && <button type="button" onClick={() => onToggleCompare(property)} disabled={compareDisabled} aria-pressed={compared} aria-label={compared ? tx(`Remove ${property.title} from comparison`, `${property.title} ကို နှိုင်းယှဉ်မှုမှဖယ်ရန်`) : tx(`Add ${property.title} to comparison`, `${property.title} ကို နှိုင်းယှဉ်ရန်ထည့်ရန်`)} className={cn("absolute bottom-3 right-3 z-20 grid size-11 place-items-center rounded-full border border-white/85 bg-white/92 text-[#0A67FF] shadow-sm backdrop-blur-md", compared && "bg-[#0A67FF] text-white", compareDisabled && "cursor-not-allowed opacity-45")}>{compared ? <Check className="size-4" /> : <Scale className="size-[18px]" />}</button>}
        </div>
      </motion.article>
    );
  }
  ...
  return (
    <motion.article initial={reduceMotion ? false : { opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-24px" }} transition={reduceMotion ? { duration: 0 } : a7Motion.slow} className="group relative overflow-hidden rounded-[26px] border border-[#E2E7EF] bg-white shadow-[0_14px_34px_rgba(26,52,88,.09)]">
      <div className="relative aspect-[1.92/1] overflow-hidden bg-[#EAF0F6]">
        <Link href={href} onClick={onOpen} className="absolute inset-0 z-10" aria-label={tx(`View ${property.title}`, `${property.title} ကိုကြည့်ရန်`)} />
        ...
        <span className="absolute left-4 top-4 z-20 inline-flex h-9 items-center gap-1.5 rounded-full border border-white/85 bg-white/92 px-3.5 text-[9px] font-semibold text-[#173257] shadow-sm backdrop-blur-md"><Check className="size-3.5 rounded-full bg-[#0A67FF] p-0.5 text-white" />{tx("Verified", "စိစစ်ပြီး")} {propertyTypeLabel}</span>
        <motion.button type="button" onClick={toggleSaved} aria-label={saved ? tx(`Remove ${property.title} from saved homes`, `${property.title} ကို သိမ်းထားမှုမှဖယ်ရန်`) : tx(`Save ${property.title}`, `${property.title} ကို သိမ်းရန်`)} aria-pressed={saved} animate={reduceMotion ? undefined : saved ? { scale: [1, 1.2, 1] } : { scale: 1 }} transition={reduceMotion ? { duration: 0 } : a7Motion.base} className="absolute right-4 top-4 z-20 grid size-12 place-items-center rounded-full border border-white/85 bg-white/92 text-[#0B1D41] shadow-sm backdrop-blur-md"><Heart className={cn("size-5", saved && "fill-current text-[#0A67FF]")} /></motion.button>

        {property.images.length > 1 && <>
          <button type="button" onClick={previousImage} className="absolute left-4 top-1/2 z-20 grid size-11 -translate-y-1/2 place-items-center rounded-full bg-white/80 text-[#173257] shadow-sm backdrop-blur-md" aria-label={tx("Previous property photo", "ယခင်အိမ်ပုံ")}><ChevronLeft className="size-5" /></button>
          <button type="button" onClick={nextImage} className="absolute right-4 top-1/2 z-20 grid size-11 -translate-y-1/2 place-items-center rounded-full bg-white/80 text-[#173257] shadow-sm backdrop-blur-md" aria-label={tx("Next property photo", "နောက်အိမ်ပုံ")}><ChevronRight className="size-5" /></button>
        </>}
        <span className="absolute bottom-4 right-4 z-20 inline-flex h-9 items-center gap-2 rounded-[12px] bg-white/88 px-3 text-[10px] font-semibold text-[#173257] shadow-sm backdrop-blur-md"><ImageIcon className="size-4" />{imageIndex + 1}/{property.images.length}</span>
      </div>
