...
function ListingEditorSheet({ open, onOpenChange, property, onSave }: ListingEditorSheetProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange} title={property ? "Edit property" : "Create a listing"} description="Add clear facts and strong photos. You can save a draft before publishing." side="right" footer={!saved && <Button className="w-full" onClick={save}>{property ? "Save changes" : "Save listing draft"}</Button>}>
      {saved ? <div className="flex min-h-[420px] flex-col items-center justify-center p-8 text-center"><span className="grid size-16 place-items-center rounded-full bg-[#DCEBFF] text-[#2D7D46]"><CheckCircle2 className="size-8" /></span><h3 className="mt-5 text-lg font-semibold">{property ? "Property updated" : "Draft created"}</h3><p className="mt-2 max-w-xs text-xs leading-5 text-[#59616A]">Your listing is saved. Complete verification before publishing it to home seekers.</p><Button variant="outline" className="mt-6" onClick={() => onOpenChange(false)}>Back to properties</Button></div> : <div className="space-y-5 p-5 sm:p-7">
        {error && <div role="alert" className="rounded-xl border border-[#EAB7B1] bg-[#FFF7F6] px-4 py-3 text-xs text-[#B13D34]">{error}</div>}
        <label className="block"><span className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-[#667085]">Property title</span><input value={draft.title} onChange={(event) => field("title", event.target.value)} placeholder="Bright 2-bed condo near Hledan" className="h-11 w-full rounded-xl border border-[#101828]/12 px-3 text-xs outline-none focus:border-[#123B73]" /></label>
        <div className="grid grid-cols-2 gap-3"><label><span className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-[#667085]">Purpose</span><select value={draft.purpose} onChange={(event) => field("purpose", event.target.value)} className="h-11 w-full rounded-xl border border-[#101828]/12 bg-[#F8FBFF] px-3 text-xs"><option value="rent">For rent</option><option value="sale">For sale</option></select></label><label><span className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-[#667085]">Property type</span><select value={draft.propertyType} onChange={(event) => field("propertyType", event.target.value)} className="h-11 w-full rounded-xl border border-[#101828]/12 bg-[#F8FBFF] px-3 text-xs">{Object.entries(propertyTypeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div>
        <label className="block"><span className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-[#667085]">Township</span><select value={draft.township} onChange={(event) => field("township", event.target.value)} className="h-11 w-full rounded-xl border border-[#101828]/12 bg-[#F8FBFF] px-3 text-xs">{searchLocations.filter((item) => !["All Myanmar", "Yangon", "Mandalay"].includes(item)).map((item) => <option key={item}>{item}</option>)}</select></label>
        <label className="block"><span className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-[#667085]">Price in MMK</span><input type="number" value={draft.price} onChange={(event) => field("price", event.target.value)} placeholder="800000" className="h-11 w-full rounded-xl border border-[#101828]/12 px-3 text-xs outline-none focus:border-[#123B73]" /></label>
        <div className="grid grid-cols-2 gap-3"><label><span className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-[#667085]">Bedrooms</span><input type="number" min="0" value={draft.bedrooms} onChange={(event) => field("bedrooms", event.target.value)} className="h-11 w-full rounded-xl border border-[#101828]/12 px-3 text-xs" /></label><label><span className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-[#667085]">Bathrooms</span><input type="number" min="0" value={draft.bathrooms} onChange={(event) => field("bathrooms", event.target.value)} className="h-11 w-full rounded-xl border border-[#101828]/12 px-3 text-xs" /></label></div>
        <div><span className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-[#667085]">Photos</span><label className="flex min-h-32 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-[#123B73]/30 bg-[#EAF4FF]/60 p-5 text-center"><input type="file" accept="image/*" multiple className="sr-only" onChange={(event) => setPhotoCount(Math.min(event.target.files?.length ?? 0, 20))} /><span className="grid size-10 place-items-center rounded-xl bg-[#F8FBFF] text-[#123B73] shadow-sm"><ImagePlus className="size-5" /></span><strong className="mt-3 text-xs">{photoCount ? `${photoCount} photo${photoCount === 1 ? "" : "s"} ready` : "Upload property photos"}</strong><small className="mt-1 text-[9px] text-[#667085]">JPG or PNG · up to 20 photos</small></label><p className="mt-2 flex items-center gap-1.5 text-[9px] text-[#667085]"><Camera className="size-3.5" />Start with a bright, wide room photo for more inquiries.</p></div>
      </div>}
    </Sheet>
  );
}
