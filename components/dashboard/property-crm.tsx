      ...
      <section id="crm-analytics" className="scroll-mt-24">
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
          ...
          <Card id="crm-verification" className="scroll-mt-24">
            <CardContent className="p-5 sm:p-6">
              <div className="flex items-start justify-between"><div><h2 className="text-lg font-semibold tracking-[-0.03em]">Verification health</h2><p className="mt-1 text-[11px] text-[#667486]">Keep every listing trusted</p></div><span className="grid size-10 place-items-center rounded-xl bg-[#EEF5FC] text-[#287A4B]"><ShieldCheck className="size-5" /></span></div>
              <div className="mt-5 space-y-2.5">
                {["Identity verified", "Phone verified", "23 listings reviewed", "2 listings need documents"].map((item, index) => (
                  <div key={item} className="flex items-center gap-3 rounded-xl border border-[#172B3F]/5 bg-[#F8FAFC] p-3 text-[11px] font-medium"><span className={`grid size-6 place-items-center rounded-full ${index === 3 ? "bg-[#FFF7E8] text-[#9A6500]" : "bg-[#EEF5FC] text-[#287A4B]"}`}>{index === 3 ? "!" : <Check className="size-3.5" />}</span>{item}</div>
                ))}
              </div>
      ...
      <Sheet open={Boolean(selectedLead)} onOpenChange={(open) => { if (!open) setSelectedLead(null); }} title={selectedLead ? `${selectedLead.name} · ${selectedLead.intent}` : "Lead details"} description={selectedLeadProperty?.title} side="right" footer={selectedLead && (
        ...
        {selectedLead && (
          <div className="space-y-5 p-5 sm:p-7">
            {leadStatus && <div role="status" className="rounded-2xl bg-[#EEF5FC] p-4 text-xs text-[#27714D]">{leadStatus}</div>}
