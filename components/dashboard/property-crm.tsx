  ...
  return (
    ...
    >
      <div id="crm-overview" className="scroll-mt-24">
        <div className="premium-surface mb-6 flex flex-wrap items-center justify-between gap-3 rounded-[20px] p-3.5 sm:p-4">
          <div className="flex items-center gap-3">
            <span className="grid size-11 place-items-center rounded-2xl bg-[linear-gradient(135deg,#EAF2FF,#D6E6FF)] text-xs font-semibold text-[#006AFF]">{profile.initials}</span>
            <span>
          ...
          <Link href={role === "owner" ? "/agent" : "/owner"} className="flex items-center gap-1 text-[11px] font-semibold text-[#006AFF]">Switch to {role === "owner" ? "agent" : "owner"} view <ChevronRight className="size-3.5" /></Link>
        </div>
        {notice && <div className="mb-5 flex items-center gap-2 rounded-xl border border-[#2D7D46]/20 bg-[#F6F8FC] px-4 py-3 text-xs text-[#006AFF]" role="status"><Check className="size-4" />{notice}</div>}
      </div>
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
          <Card>
            <CardContent className="p-5 sm:p-6">
              <div className="flex items-start justify-between"><div><h2 className="text-lg font-semibold tracking-[-0.03em]">Listing views</h2><p className="mt-1 text-[11px] text-[#667486]">Last 7 days across your active inventory</p></div><Badge className="border border-[#287A4B]/10 bg-[#F3FAF6] text-[#287A4B]"><TrendingUp className="size-3.5" />18.6%</Badge></div>
          ...
          <Card id="crm-verification" className="scroll-mt-24">
            <CardContent className="p-5 sm:p-6">
              <div className="flex items-start justify-between"><div><h2 className="text-lg font-semibold tracking-[-0.03em]">Verification health</h2><p className="mt-1 text-[11px] text-[#667486]">Keep every listing trusted</p></div><span className="grid size-10 place-items-center rounded-xl bg-[#EAF2FF] text-[#287A4B]"><ShieldCheck className="size-5" /></span></div>
              <div className="mt-5 space-y-2.5">
                {["Identity verified", "Phone verified", "23 listings reviewed", "2 listings need documents"].map((item, index) => (
                  <div key={item} className="flex items-center gap-3 rounded-xl border border-[#172B3F]/5 bg-[#F8FAFC] p-3 text-[11px] font-medium"><span className={`grid size-6 place-items-center rounded-full ${index === 3 ? "bg-[#FFF7E8] text-[#9A6500]" : "bg-[#EAF2FF] text-[#287A4B]"}`}>{index === 3 ? "!" : <Check className="size-3.5" />}</span>{item}</div>
                ))}
              </div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,.6fr)]">
        <section id="crm-properties" className="scroll-mt-24 overflow-hidden rounded-[22px] border border-[#2A2A33]/8 bg-white shadow-sm">
          <div className="flex items-end justify-between border-b border-[#2A2A33]/8 p-5 sm:p-6">
            ...
            <button type="button" onClick={() => showAll ? setShowAll(false) : openProperties()} className="text-[10px] font-semibold text-[#006AFF]">{showAll ? "Show featured" : "View all"}</button>
          </div>
          <div className="hidden max-h-[640px] overflow-auto md:block">
            <table className="w-full min-w-[700px] text-left">
              <thead className="sticky top-0 z-10 bg-[#FAFAFA] text-[9px] uppercase tracking-wider text-[#6B7078]"><tr><th className="px-5 py-3 font-semibold">Property</th><th className="px-4 py-3 font-semibold">Price</th><th className="px-4 py-3 font-semibold">Views</th><th className="px-4 py-3 font-semibold">Status</th><th className="px-4 py-3"><span className="sr-only">Actions</span></th></tr></thead>
              <tbody className="divide-y divide-[#D1D1D5]">
                {displayedProperties.map((property, index) => (
                  <tr key={property.id}>
                    ...
                    <td className="px-4 py-4"><Badge className={index === 3 ? "bg-[#F1F6FF] text-[#b77722]" : "bg-[#F6F8FC] text-[#2D7D46]"}>{index === 3 ? "Needs review" : "Active"}</Badge></td>
          ...
          <div className="max-h-[640px] divide-y divide-[#D1D1D5] overflow-auto md:hidden">
            {displayedProperties.map((property, index) => (
              <div key={property.id} className="p-4">
                ...
                <div className="mt-3 flex items-center justify-between"><span className="text-[10px] font-semibold">{formatPropertyPrice(property, isMyanmar ? "my" : "en")}</span><Badge className="bg-[#F6F8FC] text-[#2D7D46]">Active</Badge></div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="crm-messages" className="scroll-mt-24 rounded-[22px] border border-[#2A2A33]/8 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-start justify-between"><div><h2 className="text-lg font-semibold tracking-[-0.03em]">Recent leads</h2><p className="mt-1 text-[10px] text-[#6B7078]">Prioritized by intent</p></div><BarChart3 className="size-5 text-[#006AFF]" /></div>
          <div className="mt-4 divide-y divide-[#D1D1D5]">
            {crmLeads.map((lead) => (
              <button key={lead.id} type="button" onClick={() => openLead(lead)} className="flex w-full items-center gap-3 py-4 text-left">
                ...
                <Badge className={lead.status === "New" ? "bg-[#EDF4FF] text-[#006AFF]" : "bg-[#F6F8FC] text-[#006AFF]"}>{lead.status}</Badge>
              </button>
            ))}
          </div>
      ...
      <section id="crm-settings" className="scroll-mt-24 mt-5 rounded-[22px] border border-[#2A2A33]/8 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex items-start justify-between"><div><h2 className="text-lg font-semibold tracking-[-0.03em]">Workspace settings</h2><p className="mt-1 text-[10px] text-[#6B7078]">Contact details and notification preferences</p></div><Settings className="size-5 text-[#006AFF]" /></div>
        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          <label><span className="text-[10px] font-semibold text-[#5E6B79]">Display name</span><input value={settingsDraft.name} onChange={(event) => setSettingsDraft({ ...settingsDraft, name: event.target.value })} className="mt-2 h-11 w-full rounded-xl border border-[#D5DDE5] px-3 text-xs outline-none focus:border-[#006AFF]" /></label>
          <label><span className="text-[10px] font-semibold text-[#5E6B79]">Email</span><input type="email" value={settingsDraft.email} onChange={(event) => setSettingsDraft({ ...settingsDraft, email: event.target.value })} className="mt-2 h-11 w-full rounded-xl border border-[#D5DDE5] px-3 text-xs outline-none focus:border-[#006AFF]" /></label>
          <label><span className="text-[10px] font-semibold text-[#5E6B79]">Phone</span><input value={settingsDraft.phone} onChange={(event) => setSettingsDraft({ ...settingsDraft, phone: event.target.value })} className="mt-2 h-11 w-full rounded-xl border border-[#D5DDE5] px-3 text-xs outline-none focus:border-[#006AFF]" /></label>
        </div>
        <div className="mt-5 flex flex-wrap items-center gap-5 border-t border-[#E5E9ED] pt-5">
          {[["instantLeads", "Instant lead alerts"], ["weeklyReports", "Weekly performance reports"]].map(([key, label]) => (
            <label key={key} className="inline-flex items-center gap-2 text-[11px] font-medium"><input type="checkbox" checked={settingsDraft[key as "instantLeads" | "weeklyReports"]} onChange={(event) => setSettingsDraft({ ...settingsDraft, [key]: event.target.checked })} className="size-4 accent-[#006AFF]" />{label}</label>
          ))}
      ...
      <Sheet open={Boolean(selectedLead)} onOpenChange={(open) => { if (!open) setSelectedLead(null); }} title={selectedLead ? `${selectedLead.name} · ${selectedLead.intent}` : "Lead details"} description={selectedLeadProperty?.title} side="right" footer={selectedLead && (
        <form onSubmit={sendLeadReply} className="flex items-center gap-2"><input value={leadReply} onChange={(event) => setLeadReply(event.target.value)} placeholder="Write a reply…" className="h-11 min-w-0 flex-1 rounded-full border border-[#D5DDE5] px-4 text-xs outline-none focus:border-[#006AFF]" /><Button type="submit" size="icon" className="size-11 rounded-full" disabled={!leadReply.trim()} aria-label="Send lead reply"><Send className="size-4" /></Button></form>
      )}>
        {selectedLead && (
          <div className="space-y-5 p-5 sm:p-7">
            {leadStatus && <div role="status" className="rounded-2xl bg-[#EDF7F0] p-4 text-xs text-[#27714D]">{leadStatus}</div>}
            <div className="flex items-center gap-4 rounded-2xl bg-[#F4F7FA] p-4"><span className="grid size-12 place-items-center rounded-full bg-white text-xs font-semibold text-[#285575]">{selectedLead.name.split(" ").map((part) => part[0]).join("")}</span><span><strong className="block text-sm">{selectedLead.name}</strong><small className="mt-1 block text-[10px] text-[#74808C]">{selectedLead.time} · {selectedLead.status}</small></span></div>
            <div className="rounded-2xl border border-[#DCE3EA] p-4"><span className="text-[9px] font-semibold uppercase tracking-wider text-[#7B8793]">Customer intent</span><p className="mt-2 text-xs font-semibold">{selectedLead.intent}</p><p className="mt-2 text-[11px] leading-5 text-[#677482]">This home seeker is ready for a clear, timely answer. Reply through A7 to keep their contact details private.</p></div>
            {selectedLeadProperty && <Link href={`/properties/${selectedLeadProperty.id}`} className="flex items-center justify-between rounded-2xl border border-[#BCD5FA] bg-[#F5F8FD] p-4 text-xs font-semibold text-[#075FCB]">View property <ChevronRight className="size-4" /></Link>}
          </div>
        )}
      </Sheet>
      ...
      <Sheet open={leadInboxOpen} onOpenChange={setLeadInboxOpen} title="Lead inbox" description="Every active property conversation, prioritized by intent." side="right">
        <div className="p-5 sm:p-7">
          <label className="flex h-11 items-center gap-2 rounded-xl border border-[#D7DFE7] bg-[#F8FAFC] px-3"><Search className="size-4 text-[#7A8793]" /><input placeholder="Search leads" className="min-w-0 flex-1 bg-transparent text-xs outline-none" /></label>
          <div className="mt-5 divide-y divide-[#E4E8EC]">
            {crmLeads.map((lead) => <button key={lead.id} type="button" onClick={() => { setLeadInboxOpen(false); openLead(lead); }} className="flex w-full items-center gap-3 py-4 text-left"><span className="grid size-10 place-items-center rounded-full bg-[#EEF4FB] text-[10px] font-semibold">{lead.name.split(" ").map((part) => part[0]).join("")}</span><span className="min-w-0 flex-1"><strong className="block text-xs">{lead.name}</strong><small className="mt-1 block truncate text-[10px] text-[#74808C]">{lead.intent} · {lead.time}</small></span><ChevronRight className="size-4 text-[#9AA4AE]" /></button>)}
          </div>
        </div>
      </Sheet>
      ...
      <Sheet open={verificationOpen} onOpenChange={setVerificationOpen} title="Verification center" description="Resolve listing issues and keep your inventory trusted." side="right" footer={!documentsReady && <Button className="w-full" onClick={() => setDocumentsReady(true)}><Upload className="size-4" />Submit documents</Button>}>
        {documentsReady ? (
          ...
          <div className="space-y-4 p-5 sm:p-7">
            ...
            <label className="flex min-h-36 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-[#7DAFFF] bg-[#F5F8FD] p-5 text-center"><input type="file" multiple className="sr-only" /><Upload className="size-6 text-[#006AFF]" /><strong className="mt-3 text-xs">Choose verification documents</strong><small className="mt-1 text-[9px] text-[#73808C]">PDF, JPG, or PNG</small></label>
          </div>
        )}
      </Sheet>
      ...
      <Sheet open={notificationsOpen} onOpenChange={setNotificationsOpen} title="Notifications" description="Important updates across your workspace." side="right">
        <div className="divide-y divide-[#E4E8EC] p-5 sm:p-7">
          {notificationItems.map((item, index) => <button key={item.id} type="button" onClick={() => { if (item.id === "verification") { setNotificationsOpen(false); setVerificationOpen(true); } else if (item.id === "lead") { setNotificationsOpen(false); openLead(crmLeads[0]); } }} className="flex w-full items-start gap-3 py-4 text-left"><span className={cn("mt-1 size-2 shrink-0 rounded-full", index < 2 ? "bg-[#006AFF]" : "bg-[#C8D0D8]")} /><span className="min-w-0 flex-1"><strong className="block text-xs">{item.title}</strong><small className="mt-1.5 block text-[10px] leading-4 text-[#73808C]">{item.detail}</small></span><small className="shrink-0 text-[9px] text-[#8E98A2]">{item.time}</small></button>)}
        </div>
      </Sheet>
    </DashboardShell>
  );
