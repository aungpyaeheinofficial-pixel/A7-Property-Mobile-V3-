  ...
  if (status === "unauthenticated") {
    return fallback ?? (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <a href="/sign-in" className="inline-flex h-11 items-center rounded-xl bg-[#006AFF] px-5 text-sm font-semibold text-white">Sign in</a>
      </div>
    );
  }
  ...
  if (requireRole && user?.accountType !== requireRole) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 px-6 text-center">
        ...
        <a href={user?.accountType === "lister" ? "/owner" : "/dashboard"} className="inline-flex h-11 items-center rounded-xl bg-[#006AFF] px-5 text-sm font-semibold text-white">Go to your dashboard</a>
      </div>
    );
  }
