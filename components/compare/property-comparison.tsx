...
function PropertyComparison() {
  ...
  if (comparisonProperties.length < 2) {
    ...
    return (
      <div className="min-h-screen bg-[#FAF8F5] pb-28 text-[#111827]">
        ...
        <main className="mx-auto max-w-[920px] px-4 py-10 sm:px-6 sm:py-16">
          <section className="rounded-[24px] border border-[#E2DED7] bg-white p-6 text-center shadow-[0_8px_30px_rgba(15,23,42,.07)] sm:p-10">
            <span className="mx-auto grid size-14 place-items-center rounded-full bg-[#EEF5FC] text-[#0057D9]"><Scale className="size-6" /></span>
            ...
            <Link href={`/search?purpose=${selected?.purpose ?? "rent"}`} className="mt-7 inline-flex h-12 items-center gap-2 rounded-[14px] bg-[#0057D9] px-5 text-[11px] font-semibold text-white"><Scale className="size-4" />{tx("Find homes to compare", "နှိုင်းယှဉ်ရန် အိမ်ရှာမယ်")}</Link>
          </section>
        </main>
      </div>
    );
  ...
  const rows: ComparisonRow[] = [
    { label: tx("Price", "ဈေးနှုန်း"), value: (property) => <strong className="text-[#0057D9]">{formatPropertyPrice(property, isMyanmar ? "my" : "en")}{property.purpose === "rent" && <small className="ml-1 font-normal text-[#7B837F]">/ {tx("month", "လ")}</small>}</strong>, numeric: comparablePrice ? (property) => property.price : undefined, best: "low" },
    ...
    { label: tx("Verification", "စိစစ်မှု"), value: (property) => property.verification_status === "verified" ? <span className="inline-flex items-center gap-1 text-[#0057D9]"><ShieldCheck className="size-4" />{tx("A7 verified", "A7 စိစစ်ပြီး")}</span> : tx("Review pending", "စိစစ်နေဆဲ") },
...
function ComparisonHeader({ tx }: { tx: (english: string, myanmar: string) => string }) {
  return <header className="sticky top-0 z-50 border-b border-[#E2DED7] bg-[#FAF8F5]/94 backdrop-blur-2xl"><div className="mx-auto flex h-[70px] max-w-[1240px] items-center gap-3 px-4 sm:px-6 lg:px-8"><Link href="/search?purpose=rent" className="grid size-11 place-items-center rounded-full border border-[#DDD9D1] bg-white text-[#0057D9]" aria-label={tx("Back to search", "ရှာဖွေမှုသို့ပြန်ရန်")}><ArrowLeft className="size-[18px]" /></Link><Link href="/" aria-label="A7 Property home"><A7Brand /></Link><div className="ml-auto"><LanguageSwitcher compact /></div></div></header>;
}
...
function ComparisonPropertyHeader({ property, onRemove, tx, isMyanmar }: { property: Property; onRemove: () => void; tx: (english: string, myanmar: string) => string; isMyanmar: boolean }) {
  return (
    <article className="min-w-0 border-l border-[#E7E3DC] p-3">
      ...
      <p className="mt-3 flex items-center gap-1 truncate text-[8px] text-[#6E7772]"><MapPin className="size-3 text-[#0057D9]" />{property.township}, {property.city}</p>
      <h2 className="mt-1.5 line-clamp-2 min-h-10 text-[12px] font-semibold leading-5 text-[#172033]">{property.title}</h2>
      <p className="mt-2 text-[13px] font-semibold text-[#0057D9]">{formatPropertyPrice(property, isMyanmar ? "my" : "en")}</p>
