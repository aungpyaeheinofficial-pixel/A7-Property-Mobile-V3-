  ...
  return (
    <div className="min-h-screen bg-[#F7F7F4] text-[#17263A]">
      ...
      <main className="mx-auto max-w-[1120px] px-4 pb-20 pt-10 sm:px-6 sm:pt-14 lg:px-10">
        <Link href="/" className="inline-flex items-center gap-2 text-xs font-semibold text-[#014BAA]"><ArrowLeft className="size-4" />Back to A7 Property</Link>
          ...
          <section className="mt-8 grid gap-4 sm:grid-cols-3">
            {[
              ...
              <Link key={item.title} href={item.href} className="group rounded-[22px] bg-white p-5 shadow-[0_8px_28px_rgba(26,39,56,.06)] ring-1 ring-[#17263A]/7 transition-transform hover:-translate-y-0.5">
                ...
                <span className="mt-4 inline-flex items-center gap-1 text-[10px] font-semibold text-[#014BAA]">Open <ArrowRight className="size-3.5 transition-transform group-hover:translate-x-0.5" /></span>
              </Link>
            ))}
          </section>
        )}
        ...
        <div className="mt-10 grid gap-5 sm:mt-12">
          {sections.map((section, index) => (
            <section key={section.title} className="rounded-[26px] bg-white p-6 shadow-[0_10px_34px_rgba(26,39,56,.05)] ring-1 ring-[#17263A]/7 sm:p-8">
              <div className="flex items-start gap-4">
                <span className="grid size-9 shrink-0 place-items-center rounded-full bg-[#EEF5FC] text-[11px] font-semibold text-[#014BAA]">{index + 1}</span>
        ...
        <section className="mt-10 flex flex-col gap-4 rounded-[26px] border border-[#BCD5FA] bg-[#F1F6FF] p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
          ...
          <Link href="/assistant" className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-[#014BAA] px-5 text-xs font-semibold text-white">Ask A7 AI <ArrowRight className="size-4" /></Link>
        </section>
      </main>
    </div>
  );
