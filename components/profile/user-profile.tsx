...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: () => void; label: string }) {
  return (
    <button
      ...
      onClick={onChange}
      className={cn(
        "relative h-7 w-12 shrink-0 rounded-full transition-colors focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#006AFF]/20",
        checked ? "bg-[#006AFF]" : "bg-[#CCD3DB]",
      )}
    >
  ...
  return (
    <div className="min-h-screen bg-[#F7F7F4] text-[#17263A]">
        ...
        <div className="mx-auto flex h-full max-w-[1360px] items-center gap-7 px-4 sm:px-6 lg:px-10">
          ...
          <nav className="hidden items-center gap-1 md:flex" aria-label={tx("My home journey", "ကျွန်ုပ်၏ အိမ်ခရီးစဉ်")}>
            <Link href="/dashboard?section=saved" className="inline-flex h-10 items-center gap-2 rounded-full px-4 text-[12px] font-medium text-[#5F6B78] transition-colors hover:bg-white hover:text-[#17263A]">
              ...
              {savedCount > 0 && <span data-type="number" className="grid h-6 min-w-6 place-items-center rounded-full bg-[#006AFF] px-1.5 text-[10px] font-bold text-white">{savedCount}</span>}
            </Link>
          ...
          <div className="ml-auto flex items-center gap-2">
            <Link href="/search?purpose=rent" className="hidden size-10 place-items-center rounded-full text-[#526171] transition-colors hover:bg-white hover:text-[#006AFF] sm:grid" aria-label={tx("Find homes", "အိမ်ရှာရန်")}><Search className="size-[19px]" /></Link>
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-[12px] text-[#64717E]">
                <span className="inline-flex items-center gap-1.5"><MapPin className="size-4 text-[#006AFF]" />{profile.city}, Myanmar</span>
          ...
          <div className="space-y-8">
            <section className="rounded-[26px] bg-white p-5 shadow-[0_10px_34px_rgba(26,39,56,.055)] ring-1 ring-[#17263A]/7 sm:p-7">
              ...
              <div className="mt-6 divide-y divide-[#E8EAED]">
                <div className="flex items-center gap-4 py-4">
                  <span className="grid size-11 place-items-center rounded-full bg-[#EDF7F0] text-[#27714D]"><Phone className="size-[18px]" /></span>
                  ...
                  <span className="inline-flex items-center gap-1 rounded-full bg-[#EDF7F0] px-3 py-1.5 text-[9px] font-semibold text-[#27714D]"><Check className="size-3.5" />{tx("Verified", "အတည်ပြုပြီး")}</span>
                </div>
                <div className="flex items-center gap-4 py-4">
                  <span className="grid size-11 place-items-center rounded-full bg-[#EDF7F0] text-[#27714D]"><Mail className="size-[18px]" /></span>
                ...
                <div className="flex items-center gap-4 py-4">
                  <span className={cn("grid size-11 place-items-center rounded-full", identityStarted ? "bg-[#EEF4FF] text-[#006AFF]" : "bg-[#F2F3F1] text-[#64707C]")}><CircleUserRound className="size-[19px]" /></span>
                  ...
                  {identityStarted
                    ? <span className="rounded-full bg-[#EEF4FF] px-3 py-1.5 text-[9px] font-semibold text-[#006AFF]">{tx("In review", "စိစစ်နေသည်")}</span>
                    : <button type="button" onClick={() => setIdentityStarted(true)} className="rounded-full border border-[#B9D3FA] bg-white px-3 py-2 text-[9px] font-semibold text-[#006AFF] hover:bg-[#F4F8FF]">{tx("Verify identity", "အထောက်အထား စိစစ်ရန်")}</button>}
                </div>
              </div>
            </section>
            ...
            <section className="rounded-[26px] bg-white p-5 shadow-[0_10px_34px_rgba(26,39,56,.055)] ring-1 ring-[#17263A]/7 sm:p-7">
              <div className="flex items-start justify-between gap-5">
                ...
                <button type="button" onClick={openPreferenceEditor} className="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-full bg-[#F1F5F8] px-3 text-[10px] font-semibold text-[#29445F] hover:bg-[#E9F2FF] hover:text-[#006AFF]"><SlidersHorizontal className="size-3.5" />{tx("Edit", "ပြင်ရန်")}</button>
              </div>
              <dl className="mt-5 grid gap-x-8 sm:grid-cols-2">
                <PreferenceValue label={tx("Looking for", "ရှာဖွေနေသည်")}><span className="inline-flex rounded-full bg-[#EAF2FF] px-3 py-1 text-[10px] font-semibold text-[#006AFF]">{preferences.journey === "rent" ? tx("Rent", "ငှားရန်") : tx("Buy", "ဝယ်ရန်")}</span></PreferenceValue>
            ...
            <section className="rounded-[26px] bg-white p-5 shadow-[0_10px_34px_rgba(26,39,56,.055)] ring-1 ring-[#17263A]/7 sm:p-7">
              <div className="flex items-end justify-between gap-4">
                ...
                <Link href="/search?purpose=rent" className="hidden text-[10px] font-semibold text-[#006AFF] hover:underline sm:block">{tx("Create new search", "ရှာဖွေမှုအသစ်ဖန်တီးရန်")}</Link>
              </div>
              <div className="mt-6 divide-y divide-[#E8EAED]">
                {savedSearches.map((search) => (
                  <div key={search.id} className="flex items-center gap-4 py-4">
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <small className="mt-1.5 block text-[9px] font-medium text-[#006AFF]">{search.timing} {tx("alerts", "အသိပေးချက်များ")}</small>
                    </span>
                    <Toggle
              ...
              {accountSettings.map((item) => {
                ...
                return (
                  <button key={item.id} type="button" onClick={() => openSetting(item.id)} className="group flex w-full items-center gap-3 rounded-[18px] px-3 py-4 text-left transition-colors hover:bg-[#F6F7F5]">
                    <span className="grid size-10 shrink-0 place-items-center rounded-full bg-[#F1F3F5] text-[#526171] transition-colors group-hover:bg-white group-hover:text-[#006AFF]"><Icon className="size-[18px]" /></span>
      ...
      <nav className="fixed inset-x-3 bottom-2 z-50 grid h-[66px] grid-cols-4 rounded-[22px] border border-[#17263A]/10 bg-white/94 p-1.5 shadow-[0_14px_36px_rgba(23,43,63,.18)] backdrop-blur-2xl md:hidden" aria-label={tx("Mobile account navigation", "မိုဘိုင်းအကောင့် လမ်းညွှန်")}>
        ...
        <Link href="/profile" aria-current="page" className="flex flex-col items-center justify-center gap-1 rounded-2xl bg-[#EAF2FF] text-[10px] font-medium text-[#006AFF]"><UserRound className="size-[18px]" />{tx("Profile", "ကိုယ်ရေးအချက်အလက်")}</Link>
      </nav>
      ...
      >
        <div className="space-y-5 p-5 sm:p-7">
          ...
          <label className="block"><span className="text-[11px] font-semibold text-[#3D4B5B]">Full name</span><input value={profileDraft.name} onChange={(event) => setProfileDraft({ ...profileDraft, name: event.target.value })} className="mt-2 h-12 w-full rounded-xl border border-[#CCD5DF] bg-white px-4 text-sm outline-none focus:border-[#006AFF]" /></label>
          <label className="block"><span className="text-[11px] font-semibold text-[#3D4B5B]">Location</span><select value={profileDraft.city} onChange={(event) => setProfileDraft({ ...profileDraft, city: event.target.value })} className="mt-2 h-12 w-full rounded-xl border border-[#CCD5DF] bg-white px-4 text-sm outline-none focus:border-[#006AFF]"><option>Yangon</option><option>Mandalay</option></select></label>
      ...
      >
        <div className="p-5 sm:p-7">
          {settingAction && (
            <button type="button" onClick={() => { setSettingAction(null); setSettingNotice(""); }} className="mb-5 inline-flex items-center gap-1 text-[11px] font-semibold text-[#006AFF]">
          ...
          {activeSetting === "language" && (
            <div className="space-y-3">
              {([{ value: "en", label: "English" }, { value: "my", label: "မြန်မာ" }] as const).map((option) => (
                <button key={option.value} type="button" onClick={() => setLanguage(option.value)} className={cn("flex w-full items-center justify-between rounded-2xl border p-4 text-left text-sm font-semibold", language === option.value ? "border-[#006AFF] bg-[#F3F7FF] text-[#006AFF]" : "border-[#E0E4E8]")}>
          ...
          {activeSetting === "privacy" && settingAction === "download" && (
            <div className="rounded-2xl border border-[#E0E4E8] p-5">
              <Download className="size-6 text-[#006AFF]" />
          ...
          {activeSetting === "help" && settingAction === "safety" && (
            <div>
              <label className="block"><span className="text-[11px] font-semibold">Tell our trust team what happened</span><textarea value={safetyReport} onChange={(event) => setSafetyReport(event.target.value)} className="mt-2 min-h-36 w-full resize-none rounded-2xl border border-[#CCD5DF] p-4 text-xs leading-5 outline-none focus:border-[#006AFF]" placeholder="Include the listing, person, and anything that made you feel unsafe." /></label>
