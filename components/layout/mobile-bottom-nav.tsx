...
function MobileBottomNav() {
  ...
  return (
    <nav
      className="fixed inset-x-3 bottom-[max(.625rem,env(safe-area-inset-bottom))] z-[80] mx-auto grid h-[68px] max-w-[520px] grid-cols-5 rounded-[32px] border border-white/12 bg-[rgba(8,10,14,.82)] px-1.5 shadow-[0_12px_35px_rgba(0,0,0,.28),inset_0_1px_0_rgba(255,255,255,.10)] backdrop-blur-[24px] backdrop-saturate-[1.4] lg:hidden"
        ...
        return (
          <Link
            ...
            className={cn(
              ...
              active ? "text-white" : "text-[#777C85] hover:text-white",
            )}
          >
            {active && (
              <motion.span
            ...
            <motion.span animate={reduceMotion ? undefined : { y: active ? -8 : 0, scale: active ? 1.06 : 1 }} transition={reduceMotion ? { duration: 0 } : a7Motion.spring} className="relative z-10 grid place-items-center">
              <Icon className={cn("size-[20px] transition-[color,fill] duration-200", active ? "text-white" : "text-[#7E838C]", active && item.id === "saved" && "fill-current")} strokeWidth={active ? 2.3 : 1.9} />
            </motion.span>
            <motion.span animate={reduceMotion ? undefined : { y: active ? -6 : 0, opacity: active ? 1 : 0.82 }} transition={reduceMotion ? { duration: 0 } : a7Motion.base} className={cn("relative z-10 max-w-full truncate px-0.5", active ? "text-white" : "text-[#777C85]")}>{isMyanmar ? item.labelMy : item.label}</motion.span>
