  ...
  return (
    <>
      <section className="relative grid h-[310px] grid-cols-1 gap-2 overflow-hidden rounded-[22px] sm:h-[430px] md:grid-cols-2 lg:h-[500px] lg:grid-cols-[1.25fr_.75fr]" aria-label="Property photos">
        <button className="relative overflow-hidden bg-[#e6f4fb] text-left" onClick={() => view(0)} aria-label="Open main property photo">
        ...
        <div className="hidden grid-cols-2 gap-2 md:grid">
          {gallery.slice(1, 5).map((image, index) => <button key={`${image}-${index}`} className="relative overflow-hidden bg-[#e6f4fb]" onClick={() => view(index + 1)} aria-label={`Open property photo ${index + 2}`}><Image src={image} alt={`${title} — view ${index + 2}`} fill sizes="20vw" className="object-cover transition-transform duration-500 hover:scale-[1.03]" /></button>)}
        </div>
      ...
      <Sheet open={open} onOpenChange={setOpen} title="Property gallery" description={`${gallery.length} professional property photos`} side="bottom" className="h-[94vh] max-h-none">
        <div className="mx-auto grid h-full max-w-6xl grid-rows-[1fr_auto] gap-4 p-4 sm:p-6">
          <div className="relative min-h-0 overflow-hidden rounded-[18px] bg-[#172b3f]">
