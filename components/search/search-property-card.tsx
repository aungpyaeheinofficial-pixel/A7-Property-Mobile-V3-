  ...
  return (
    <article
      className={cn(
        ...
        selected ? "border-[#0057D9] ring-2 ring-[#0057D9]/15" : "border-[#172B3F]/9",
      )}
      onMouseEnter={() => onFocus?.(property)}
      onFocus={() => onFocus?.(property)}
    >
      <Link href={href} className="absolute inset-0 z-10 rounded-2xl focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-[#0057D9]/35" aria-label={tx(`View details for ${property.title}`, `${property.title} အသေးစိတ်ကြည့်ရန်`)} />

      <div className="relative min-h-[205px] overflow-hidden bg-[#EEF5FC] sm:min-h-[260px]">
      ...
      <div className="flex min-w-0 flex-col p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="shrink-0 rounded-lg bg-[#F1F6FF] px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-[.08em] text-[#0057D9]">{property.purpose === "rent" ? tx("For rent", "ငှားရန်") : tx("For sale", "ရောင်းရန်")}</span>
        </div>
        ...
        <p className="mt-1.5 flex items-center gap-1.5 text-xs font-medium text-[#59616A]"><MapPin className="size-4 shrink-0 text-[#0057D9]" /><span className="truncate">{property.township}, {property.city} · {meta.ward}</span></p>
        ...
        <div className="mt-3.5 flex items-center justify-between gap-3 rounded-xl border border-[#D9E8FF] bg-[linear-gradient(135deg,#F7FAFF,#EEF5FF)] px-3.5 py-2.5 text-[11px]">
          ...
          <span className="shrink-0 font-semibold text-[#0057D9]">{meta.trustScore}% {tx("trust", "ယုံကြည်မှု")}</span>
        </div>
        ...
        <div className="mt-auto flex items-center justify-between gap-3 pt-3.5 text-[11px] font-medium text-[#667486]">
          ...
          <span className="inline-flex shrink-0 items-center gap-1 font-semibold text-[#0057D9]">{tx("View details", "အသေးစိတ်ကြည့်ရန်")}<ArrowRight className="size-4 transition-transform group-hover:translate-x-0.5" /></span>
        </div>
      </div>
    </article>
  );
...
function TrustBadge({ children }: { children: React.ReactNode }) {
  return <span className="inline-flex items-center gap-1.5 rounded-lg border border-white/80 bg-white/94 px-2.5 py-1.5 text-[11px] font-semibold text-[#0057D9] shadow-sm backdrop-blur"><ShieldCheck className="size-4" />{children}</span>;
}
