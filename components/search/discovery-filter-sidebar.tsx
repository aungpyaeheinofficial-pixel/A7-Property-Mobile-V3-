  ...
  return (
    <div className={cn("space-y-7", className)}>
      <div className="flex items-center justify-between">
        ...
        <button type="button" className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#006AFF] hover:text-[#0057D9]" onClick={() => onChange(clearAdvancedFilters(value))}>
          <RotateCcw className="size-4" />{tx("Reset", "ပြန်လည်သတ်မှတ်ရန်")}
        </button>
      </div>

      <FilterGroup title={tx("Furniture & parking", "ပရိဘောဂနှင့် ကားပါကင်")}>
        <select className="h-12 w-full rounded-xl border border-[#B9BEC4] bg-white px-3 text-xs focus:border-[#006AFF] focus-visible:!outline-none" value={value.furniture} onChange={(event) => set("furniture", event.target.value as DiscoveryFilters["furniture"])} aria-label={tx("Furniture", "ပရိဘောဂ")}>
        ...
        <label className="mt-2 flex cursor-pointer items-center justify-between rounded-lg border border-[#D1D1D5] bg-white p-3">
          <span className="flex items-center gap-2 text-xs font-medium"><CarFront className="size-[18px] text-[#006AFF]" />{tx("Parking required", "ကားပါကင်လိုအပ်သည်")}</span>
...
function CheckRow({ label, selected, onClick }: { label: string; selected: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} aria-pressed={selected} className="flex w-full items-center gap-2.5 rounded-lg px-1 py-2.5 text-left text-xs text-[#4A4A4F] hover:bg-[#F6F8FC]">
      <span className={cn("grid size-[18px] place-items-center rounded border", selected ? "border-[#006AFF] bg-[#006AFF] text-white" : "border-[#B9BEC4] bg-white")}>{selected && <Check className="size-3.5" />}</span>
      {label}
    </button>
  );
}
