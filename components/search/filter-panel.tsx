  ...
  return (
    <div className={cn("space-y-6", className)}>
      <div className="flex items-center justify-between">
        ...
        <button className="inline-flex items-center gap-1.5 text-[10px] font-semibold text-[#0057D9]" onClick={() => onChange(defaultSearchFilters)}><RotateCcw className="size-3.5" />Reset</button>
      </div>
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-2 rounded-xl bg-[#E8E8EB] p-1">
          {(["rent", "sale"] as const).map((purpose) => <button key={purpose} className={cn("h-9 rounded-[9px] text-xs font-semibold capitalize transition-all", value.purpose === purpose ? "bg-white text-[#0057D9] shadow-sm" : "text-[#6B7078]")} onClick={() => onChange({ ...value, purpose, minPrice: null, maxPrice: null })}>{purpose === "sale" ? "Buy" : "Rent"}</button>)}
        </div>
      </fieldset>

      <label className="block">
        <span className="mb-2.5 block text-[10px] font-semibold uppercase tracking-[0.09em] text-[#6B7078]">Location</span>
        <select className="h-11 w-full rounded-xl border border-[#2A2A33]/12 bg-white px-3 text-xs outline-none focus:border-[#0057D9] focus:ring-3 focus:ring-[#0057D9]/10" value={value.location} onChange={(event) => set("location", event.target.value)}>
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-2 gap-2">
          <label><span className="sr-only">Minimum price</span><select className="h-11 w-full rounded-xl border border-[#2A2A33]/12 bg-white px-2.5 text-[11px] outline-none focus:border-[#0057D9]" value={value.minPrice ?? ""} onChange={(event) => set("minPrice", event.target.value ? Number(event.target.value) : null)}>{priceOptions.map((price) => <option key={`min-${price}`} value={price ?? ""}>{price === null ? "Min price" : formatOption(price)}</option>)}</select></label>
          <label><span className="sr-only">Maximum price</span><select className="h-11 w-full rounded-xl border border-[#2A2A33]/12 bg-white px-2.5 text-[11px] outline-none focus:border-[#0057D9]" value={value.maxPrice ?? ""} onChange={(event) => set("maxPrice", event.target.value ? Number(event.target.value) : null)}>{priceOptions.map((price) => <option key={`max-${price}`} value={price ?? ""}>{price === null ? "Max price" : formatOption(price)}</option>)}</select></label>
        </div>
      </fieldset>
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-2 gap-2">
          {(Object.entries(propertyTypeLabels) as Array<[Property["property_type"], string]>).map(([type, label]) => {
            ...
            return <button key={type} className={cn("flex min-h-10 items-center justify-between rounded-xl border px-3 text-left text-[11px] font-medium transition-colors", selected ? "border-[#0057D9] bg-[#FAF8F5] text-[#0057D9]" : "border-[#2A2A33]/10 bg-white text-[#59616A]")} onClick={() => togglePropertyType(type)}>{label}{selected && <Check className="size-3.5" />}</button>;
          })}
        </div>
      </fieldset>
      ...
      {(["bedrooms", "bathrooms"] as const).map((field) => (
        <fieldset key={field}>
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {[null, 1, 2, 3, 4].map((count) => <button key={`${field}-${count}`} className={cn("h-9 rounded-[10px] border text-[11px] font-semibold", value[field] === count ? "border-[#0057D9] bg-[#0057D9] text-white" : "border-[#2A2A33]/10 bg-white text-[#59616A]")} onClick={() => set(field, count)}>{count === null ? "Any" : count === 4 ? "4+" : count}</button>)}
          </div>
        </fieldset>
      ))}
      ...
      <label className="block">
        ...
        <select className="h-11 w-full rounded-xl border border-[#2A2A33]/12 bg-white px-3 text-xs outline-none focus:border-[#0057D9]" value={value.furniture} onChange={(event) => set("furniture", event.target.value as SearchFilters["furniture"])}>
      ...
      <label className="flex cursor-pointer items-center justify-between rounded-xl border border-[#2A2A33]/10 bg-white p-3">
        <span className="flex items-center gap-2 text-xs font-medium"><CarFront className="size-[18px] text-[#0057D9]" />Parking required</span>
