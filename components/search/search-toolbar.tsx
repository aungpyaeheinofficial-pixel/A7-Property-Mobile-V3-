  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#2A2A33]/8 pb-4">
      ...
      <div className="flex items-center gap-2">
        <Button variant="outline" className="relative h-10 px-3 text-xs lg:hidden" onClick={onOpenFilters}><SlidersHorizontal className="size-4" />Filters{activeFilters > 0 && <span className="grid size-5 place-items-center rounded-full bg-[#0057D9] text-[9px] text-white">{activeFilters}</span>}</Button>
        <label className="hidden sm:block"><span className="sr-only">Sort properties</span><select value={sort} onChange={(event) => onSortChange(event.target.value as PropertySort)} className="h-10 rounded-xl border border-[#2A2A33]/12 bg-white px-3 text-xs font-medium outline-none focus:border-[#0057D9]"><option value="recommended">Recommended</option><option value="newest">Newest</option><option value="price-asc">Price low–high</option></select></label>
